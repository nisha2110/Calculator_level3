...
from calculator.operations import addition1, subtract1

@pytest.fixture
def setup_calculations():
    """Clear history and add sample calculations for tests."""
    Calculations.clear_history()
    Calculations.add_calculation(Calculation(Decimal('10'), Decimal('5'), addition1))
    Calculations.add_calculation(Calculation(Decimal('20'), Decimal('3'), subtract1))

def test_add_calculation(setup_calculations):
    """Test adding a calculation to the history."""
    calc = Calculation(Decimal('2'), Decimal('2'), addition1)
...
def test_find_by_operation(setup_calculations):
    ...
    add_operations = Calculations.find_by_operation("addition1")
    assert len(add_operations) == 1, \
        "Did not find the correct number of calculations with add operation"
    subtract_operations = Calculations.find_by_operation("subtract1")
